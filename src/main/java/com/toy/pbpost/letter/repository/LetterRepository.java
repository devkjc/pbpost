...
public interface LetterRepository extends JpaRepository<Letter, Long> {
    ...
    @Query(value = "select count(*) from postbird.letter l where l.bird_id_fk = :birdId and l.return_time >= :now ", nativeQuery = true)
    Long countByBirdWorking(@Param(value = "birdId") long birdId, @Param(value = "now") LocalDateTime now);

    @Query(value = "select * from postbird.letter l where l.from_uid_fk = :uid and l.return_time >= :now ", nativeQuery = true)
